...
parser.add_option("-n", "--nepochs",
                  action="store", 
                  type="int",
                  dest="epochs", 
                  help="number of epochs")

parser.add_option("-t", "--testsize",
                  action="store", 
                  type="float",
                  dest="testsize", 
                  default = 0.2,
...
parser.add_option("-a", "--augmentation",
                  ...
                  dest="augmentation", 
                  default = 0,
                  help="augmentation time")


(options, args) = parser.parse_args()

print(options.experiment)
print(options.learningrate)
...
with open(name_experiment + "/configuration.txt",'w') as out:
    ...
    line5 = 'optimizer = ' + optimizer
    line6 = 'learning rate = ' + str(learningrate)
    out.write('{}\n{}\n{}\n{}\n{}\n{}\n{}'.format(line0,line1,line2,line3,line4,line5,line6)) 
...
raw_train, raw_test, mask_train, mask_test = train_test_split(raw_img, mask_img, test_size = options.testsize, random_state = 123)
...
model = get_unet_trainable_seg(n_ch, patch_height, patch_width)  #the U-net model
...
model.compile(optimizer=myopt, loss=log_dice_loss ,metrics=['accuracy', mean_IOU_gpu, dice_metric])
...
history = model.fit(patches_imgs_train, patches_masks_train, nb_epoch=N_epochs, batch_size=batch_size, verbose=2, shuffle=True, validation_split=0.1, callbacks=[checkpointer])

pd.DataFrame.from_dict(history.history).to_csv('./'+name_experiment+'/'+ experiment +'_history.csv')
#pd.DataFrame(history.history.to_csv('./'+name_experiment+'/'+ experiment +'_history.csv'))
...
plt.ylabel('dice coefficcient')
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper left')
plt.savefig('./'+name_experiment+'/'+ experiment +'_dice_history.png')
...
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper left')
plt.savefig('./'+name_experiment+'/'+ experiment +'_loss_history.png')
...
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper left')
plt.savefig('./'+name_experiment+'/'+ experiment +'_IOU_history.png')
...
predictions = model.predict(test_imgs, batch_size=32, verbose=2)

write_hdf5(predictions, path_experiment + name_experiment + '/' +  name_experiment +  '_prediction.h5')
...
N_visual = 5
visualize(group_images(orig_imgs[0:Imgs_to_test,:,:,:],N_visual),output_path + "all_originals")#.show()
visualize(group_images(pred_imgs[0:Imgs_to_test,:,:,:],N_visual),output_path + "all_predictions")#.show()
visualize(group_images(gtruth_masks[0:Imgs_to_test,:,:,:],N_visual),output_path+ "all_groundTruths")#.show()
#visualize results comparing mask and prediction:
assert (orig_imgs.shape[0]==pred_imgs.shape[0] and orig_imgs.shape[0]==gtruth_masks.shape[0])
N_predicted = Imgs_to_test
group = N_visual
...
N_visual = 5
visualize(group_images(orig_imgs[0:Imgs_to_test,:,:,:],N_visual),output_path + "DRIVE_all_originals")#.show()
visualize(group_images(pred_imgs[0:Imgs_to_test,:,:,:],N_visual),output_path + "DRIVE_all_predictions")#.show()
#visualize(group_images(gtruth_masks[0:Imgs_to_test,:,:,:],N_visual),output_path+ "DRIVE_all_groundTruths")#.show()
#visualize results comparing mask and prediction:
...
for i in range(int(N_predicted/group)):
    ...
    #total_img = np.concatenate((orig_stripe,masks_stripe,pred_stripe),axis=0)
    total_img = np.concatenate((orig_stripe,pred_stripe),axis=0)
    visualize(total_img,output_path + "DRIVE_Original_GroundTruth_Prediction"+str(i))#.show()

    
print("Test on STARE dataset ...")
...
N_visual = 5
visualize(group_images(orig_imgs[0:Imgs_to_test,:,:,:],N_visual),output_path + "STARE_all_originals")#.show()
visualize(group_images(pred_imgs[0:Imgs_to_test,:,:,:],N_visual),output_path + "STARE_all_predictions")#.show()
#visualize(group_images(gtruth_masks[0:Imgs_to_test,:,:,:],N_visual),output_path+ "STARE_all_groundTruths")#.show()
#visualize results comparing mask and prediction:
assert (orig_imgs.shape[0]==pred_imgs.shape[0])
N_predicted = Imgs_to_test
group = N_visual
...
for i in range(int(N_predicted/group)):
    ...
    #total_img = np.concatenate((orig_stripe,masks_stripe,pred_stripe),axis=0)
    total_img = np.concatenate((orig_stripe,pred_stripe),axis=0)
    visualize(total_img,output_path + "STARE_Original_GroundTruth_Prediction"+str(i))#.show()
